    ...
    public emparejamiento(){
        while (!this.compras.isEmpty() && !this.ventas.isEmpty()){
            ...
            if (this.compras.checkMax() && this.ventas.checkMin() && this.compras.checkMax()!.getPrecio() >= this.ventas.checkMin()!.getPrecio()){
                ...
                this.historialTransacciones.push("Transaccion: Simbolo" + this.compras.checkMax()?.getSimbolo() + " - Nombre: " + this.compras.checkMax()?.getNombre() + " - Cantidad" + String(cantidadInterambiada) + " - Precio" + String(this.compras.checkMax()?.getPrecio()))
